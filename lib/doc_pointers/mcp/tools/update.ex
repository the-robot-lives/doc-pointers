defmodule DocPointers.MCP.Tools.Update do
  use Noizu.MCP.Server.Tool,
    ...
    description: "Update metadata for an existing doc-pointer (description, class, line, file path).",
    annotations: [destructive_hint: true, idempotent_hint: true]

  input do
    field :uuid, :string,
      description: "Full UUID of the pointer to update"
    field :token, :string,
      description: "4-character token of the pointer (alternative to UUID)"
    field :description, :string,
      description: "New description"
    field :class, :string,
      description: "New class/module name"
    field :line, :integer,
      description: "Updated line number"
    field :file_path, :string,
      description: "Updated file path (if the source file moved)"
  end
  ...
  def call(args, _ctx) do
    ...
    if is_nil(uuid) do
      ...
      updates = %{}
      updates = if args[:description], do: Map.put(updates, :description, args.description), else: updates
      ...
      updates = if args[:file_path], do: Map.put(updates, :file_path, args.file_path), else: updates
